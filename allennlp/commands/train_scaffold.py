...
def train_model(params: Params, serialization_dir: str) -> Model:
    ...
    if aux_train_size < len(train_data.instances):
        ...
        aux_train_data = Dataset(aux_train_data.instances + aux_sample)
    ...
    datasets_for_vocab_creation = set(params.pop("datasets_for_vocab_creation",
                                                 all_datasets))
    ...
    default_vocab_dataset = Dataset([instance for key, dataset in all_datasets.items()
                                     for instance in dataset.instances
                                     if key in datasets_for_vocab_creation])
    vocab = Vocabulary.from_params(params.pop("vocabulary", {}),
                                   dataset=default_vocab_dataset,
                                   aux_dataset=aux_train_data)
    vocab.save_to_files(os.path.join(serialization_dir, "vocabulary"))

    model = Model.from_params(vocab, params.pop('model'))
    train_data.index_instances(vocab)
    aux_train_data.index_instances(vocab)
    if validation_data:
